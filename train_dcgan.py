...
class DCGAN():
    ...
    def build_discriminator(self):
        img_shape = (self.img_width, self.img_height, self.img_depth)
    ...
    def train(self, epochs, batch_size=12, save_interval=50, data_dir=None, model_path=None):
        image_list = []
        for filename in glob.glob(f'{data_dir}/*.png'):
            im = Image.open(filename)
            reim = im.resize((self.img_width, self.img_height), Image.ANTIALIAS)
            image_list.append(reim)

        image_stack = numpy.asarray(image_list[0])  # first image in stack
